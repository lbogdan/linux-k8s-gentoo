...
GIT_COMMIT_DATE=2022-08-29T17:02:20Z
...
src_prepare() {
	...
	sed -e "s|gitCommit := unknown|gitCommit := \"${EGIT_COMMIT}\"|" \
		-e "s|gitCommitDate := unknown|gitCommitDate := \"${GIT_COMMIT_DATE}\"|" \
		-i internal/version/version.go || die
...
src_install() {
	...
	keepdir /etc/crio
	sed -E -e 's|\broot = ".+"|root = "/var/lib/containers/storage"|' \
		-e 's|\brunroot = ".+"|runroot = "/run/containers/storage"|' \
		-e 's|\bstorage_driver = ".+"|storage_driver = ""|' \
		-i "${ED}/etc/crio/crio.conf"
	...
	newins "${FILESDIR}/containers-policy.json" policy.json
	newins "${FILESDIR}/containers-registries.conf" registries.conf
}
